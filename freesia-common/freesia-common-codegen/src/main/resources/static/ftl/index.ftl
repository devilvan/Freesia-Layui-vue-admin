...
<!--                  :options="${dataBaseDto.className?uncap_first}DurationTypeSelectList"-->
<!--                  :items="${dataBaseDto.className?uncap_first}DurationTypeSelectList"-->
<!--                  :allow-clear="true"-->
<!--              ></lay-select>-->
<!--            </lay-form-item>-->
<!--          </lay-col>-->
...
<!--    </lay-card>-->
  ...
  <lay-layer v-model="showModalFlag" :area="['1200px']" :title="saveModalTitle">
   <div style="padding: 20px" @keydown.enter.prevent="toSubmit(false)" @keydown.esc.prevent="toCancel">
    <lay-form ref="saveFormRef" :model="save${dataBaseDto.className}Vo" :rules="saveFromRules" label-position="top">
     <lay-row :space="20">
      <#list dataBaseDto.fieldList as field>
       <lay-col :md="6">
        <lay-form-item label="${field.remark ! ''}" prop="${field.fieldName}">
          <lay-input
                  v-model="save${dataBaseDto.className}Vo.${field.fieldName}"
                  :allow-clear="true"
                  size="sm"
          ></lay-input>
        </lay-form-item>
...
import {delete${dataBaseDto.className}, findPage${dataBaseDto.className}} from "@/api/${module}/${dataBaseDto.className}";
...
import {find${dataBaseDto.className}, saveUpdate} from "@/api/${module}/${dataBaseDto.className}";
...
/* VAR*/
const searchQuery = ref<${dataBaseDto.className}Vo>({})
const pageQuery = reactive<PageQuery>({
 current: 1,
 limit: 10
})
const dataSource = ref<Array<${dataBaseDto.className}Entity>>()
const selectedKeys = ref<Array<string>>([])
const columns = ref([
 {title: '选项', width: '55px', type: 'checkbox', fixed: 'left'},
 <#list dataBaseDto.fieldList as field>
 {title: '${field.remark ! ''}', width: '130px', key: '${field.fieldName}'},
 </#list>
 {
  title: '操作',
  width: '150px',
...
const save${dataBaseDto.className}Vo = ref<${dataBaseDto.className}Vo>(<${dataBaseDto.className}Vo>{})
const queryFormRef = ref(null)
const ${dataBaseDto.className?uncap_first}DurationTypeSelect = ref<Array<SysDictValueEntity>>(<Array<SysDictValueEntity>>[]);
const ${dataBaseDto.className?uncap_first}DurationTypeSelectList = ref();
...
const loadDataSource = () => {
 findPage${dataBaseDto.className}(searchQuery.value, pageQuery).then((res: TableResult<${dataBaseDto.className}Entity>) => {
  if (res.code == 200) {
...
const sortChange = (key: any, sort: number) => {
 layer.msg(`字段<#noparse>${</#noparse>key<#noparse>}</#noparse> - 排序<#noparse>${</#noparse>sort}<#noparse>}</#noparse>, 你可以利用 sort-change 实现服务端排序`)
}
...
const showSaveModal = (text: any, row: any) => {
 ...
 if (row != null) {
  save${dataBaseDto.className}Vo.value = {...row}
 }
  if (Operate.EDIT === text) {
   find${dataBaseDto.className}({
   id: row.id
  }).then((res: any) => {
   if (res.code === 200) {
    save${dataBaseDto.className}Vo.value = res.data;
   }
  })
  ...
  btn: [
   {
    text: '确定',
    callback: (id: any) => {
     delete${dataBaseDto.className}(selectedKeys.value).then((res: any) => {
     if (res.code === 200) {
     layer.msg('删除成功')
    }
     loadDataSource();
...
function toSubmit(clickFlag: boolean) {
 saveFormRef.value.validate((isValidate: any, model: any, errors: any) => {
  if (isValidate) {
   saveUpdate(save${dataBaseDto.className}Vo.value).then((res: any) => {
    if (res.code === 200) {
     loadDataSource();
     layer.msg('保存成功！', {icon: 1, time: 1000})
     save${dataBaseDto.className}Vo.value = {};
     if (clickFlag) {
      showModalFlag.value = false
     } else {
      // 如果是修改+回车，则关闭窗口
      if (save${dataBaseDto.className}Vo.id && save${dataBaseDto.className}Vo.id != 0) {
       showModalFlag.value = false
      }
     }
     showModalFlag.value = false
...
function confirm(row: any) {
 if (row) {
  delete${dataBaseDto.className}([row.id]).then((res: any) => {
   if (res.code === 200) {
    layer.msg('删除成功')
   }
   loadDataSource();
