...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenuPo> implements SysMenuService {
    ...
    @Override
    public List<String> findSelectedMenuListByRoleId(Long roleId) {
        ...
        transactionTemplate.execute(status -> {
            ...
            try {
                ...
                saveSysSensitiveLogBean = USecurity.recordSensitiveLog(sysSensitiveLogBean -> {
                    ...
                    sysSensitiveLogBean.setContextOld("分配前菜单ID：" + UJSON.toJSONString(beforeAssignButtonIdList));
                    sysSensitiveLogBean.setContext("分配后菜单ID：" + UJSON.toJSONString(assignButtonIdList));
